...
export type NavigationButtonComponentProps<E extends NativeScriptNavigationButton = NativeScriptNavigationButton> = Props /* & typeof RCTNavigationButton.defaultProps */ & Partial<NavigationButtonProps> & ActionItemComponentProps<E>;
 ...
 */
export class _NavigationButton<P extends NavigationButtonComponentProps<E>, S extends {}, E extends NativeScriptNavigationButton> extends _ActionItem<P, S, E> {
    render(){
        const {
            forwardedRef,
        ...
        if(Array.isArray(children) || typeof children === "string" || typeof children === "number"){
            throw new Error(`'children' property passed into NavigationButton must be a single child node, which must not be a number or string`);
        }
...
type OwnPropsWithoutForwardedRef = PropsWithoutForwardedRef<NavigationButtonComponentProps<NativeScriptNavigationButton>>;

export const NavigationButton: React.ComponentType<OwnPropsWithoutForwardedRef & React.ClassAttributes<NativeScriptNavigationButton>> = React.forwardRef<NativeScriptNavigationButton, OwnPropsWithoutForwardedRef>(
    (props: React.PropsWithChildren<OwnPropsWithoutForwardedRef>, ref: React.RefObject<NativeScriptNavigationButton>) => {
